...
def convert_to_grayscale(image_path):
    try:
        ...
        img.convert('L').save(grayscale_path)
...
def analyze_text_with_openai(extracted_text, image_path):
    ...
    question = (
    "Please analyze the following post from Facebook. If the author is expressing a need for a realtor, respond with 'send'; if not, respond with 'delete'. Here is the post:\n\n"
    )
    ...
    try:
        ...
        if action.lower() == "send":
            brightened_img = brighten_image(image_path)
            if brightened_img:
                send_email(image_path, brightened_img)
